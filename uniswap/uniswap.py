...
from interactions import Route
...
class Uniswap:
    ...
    def get_price_input(
        ...
        if token0 == ETH_ADDRESS:
            return self._get_eth_token_input_price(token1, Wei(qty), fee,route=route)
        elif token1 == ETH_ADDRESS:
            return self._get_token_eth_input_price(token0, qty, fee,route=route)
        else:
            return self._get_token_token_input_price(token0, token1, qty, fee, route=route)
    ...
    def get_price_output(
        ...
        if is_same_address(token0, ETH_ADDRESS):
            return self._get_eth_token_output_price(token1, qty, fee, route=route)
        elif is_same_address(token1, ETH_ADDRESS):
            return self._get_token_eth_output_price(token0, Wei(qty), fee, route=route)
        else:
            return self._get_token_token_output_price(token0, token1, qty, fee, route=route)
        ...
        if route is None:
            if self.version == 2:
                ...
                if is_same_address(token0, self.get_weth_address()):
                    return int(self._get_eth_token_input_price(token1, Wei(qty), fee,route=route))
                elif is_same_address(token1, self.get_weth_address()):
                    return int(self._get_token_eth_input_price(token0, qty, fee,route=route))
    ...
    def _get_eth_token_output_price(
        ...
        if route is None:
            if self.version == 1:
                ...
                price = self.router.functions.getAmountsIn(qty, route=route).call()[0]
            elif self.version == 3:
                if not fee:
                    logger.warning("No fee set, assuming 0.3%")
                    fee = 3000
                price = Wei(
                    self._get_token_token_output_price(
                        self.get_weth_address(), token, qty, fee=fee,route=route
                    )
                )
        else:
            price = self.router.functions.getAmountsIn(qty, route=route).call()[0]
    ...
    def _get_token_eth_output_price(
        ...
        qty: Wei, 
        fee: int = None ,
        route: Optional[List[AddressLike]] = None # input token
    ) -> int:
    ...
    def _get_token_token_output_price(
        ...
        if self.version == 2:
            price: int = self.router.functions.getAmountsIn(qty, route=route).call()[0]
        elif self.version == 3:
            if not fee:
                logger.warning("No fee set, assuming 0.3%")
                fee = 3000
    ...
    def make_trade_output(
        ...
        if input_token == ETH_ADDRESS:
            ...
            need = self._get_eth_token_output_price(output_token, qty,route=route)
            if balance < need:
    ...
    def _eth_to_token_swap_input(
        ...
        if self.version == 1:
            ...
            amount_out_min = int(
                (1 - slippage) * self._get_eth_token_input_price(output_token, qty, fee,route=route)
            )
            if fee_on_transfer:
                func = (
            ...
            min_tokens_bought = int(
                (1 - slippage)
                * self._get_eth_token_input_price(output_token, qty, fee=fee,route=route)
            )
            sqrtPriceLimitX96 = 0
    ...
    def _eth_to_token_swap_input_not_sent(
        self,
        output_token: AddressLike,
        qty: Wei,
        recipient: Optional[AddressLike],
        ...
        if self.version == 1:
            ...
            if not recipient:
                ...
                func_params.append(recipient)
                function = token_funcs.ethToTokenTransferInput(*func_params)
            return self._build_tx(function, tx_params)
    ...
    def _token_to_eth_swap_input(
        ...
        if self.version == 1:
            ...
            if recipient is None:
                recipient = self.address
            amount_out_min = int(
                (1 - slippage) * self._get_token_eth_input_price(input_token, qty, fee, route=route)
            )
            if fee_on_transfer:
                func = (
            ...
            min_tokens_bought = int(
                (1 - slippage)
                * self._get_token_eth_input_price(input_token, qty, fee=fee,route=route)
            )
            sqrtPriceLimitX96 = 0
    ...
    def _token_to_eth_swap_input_not_sent(
        self,
        input_token: AddressLike,
        qty: int,
        recipient: Optional[AddressLike],
        ...
        if route == None:
            route = [input_token, self.get_weth_address()]
        ...
        if self.version == 1:
            ...
            if not recipient:
                ...
                func_params.append(recipient)
                function = token_funcs.tokenToEthTransferInput(*func_params)
            return self._build_tx(function)
        elif self.version == 2:
            if recipient is None:
                recipient = self.address
            amount_out_min = int(
    ...
    def _token_to_token_swap_input_not_sent(
        self,
        input_token: AddressLike,
        output_token: AddressLike,
        qty: int,
    ...
    def _eth_to_token_swap_output(
        ...
        # Balance check
        eth_balance = self.get_eth_balance()
        cost = self._get_eth_token_output_price(output_token, qty, fee,route=route)
        ...
        if self.version == 1:
            token_funcs = self._exchange_contract(output_token).functions
            eth_qty = self._get_eth_token_output_price(output_token, qty,route=route)
            ...
            eth_qty = int(
                (1 + slippage)
                * self._get_eth_token_output_price(output_token, qty, fee,route=route)
            )  # type: ignore
            return self._build_and_send_tx(
    ...
    def _token_to_eth_swap_output(
        ...
        # Balance check
        input_balance = self.get_token_balance(input_token)
        cost = self._get_token_eth_output_price(input_token, qty, fee,route=route)
    ...
    def _token_to_token_swap_output(
        ...
        # Balance check
        input_balance = self.get_token_balance(input_token)
        cost = self._get_token_token_output_price(input_token, output_token, qty, fee,route=route)
        amount_in_max = int((1 + slippage) * cost)
        if (
    ...
    def approve_not_sent(self, token: AddressLike, max_approval: Optional[int] = None) -> None:
        """Give an exchange/router max approval of a token."""
        max_approval = self.max_approval_int if not max_approval else max_approval
        contract_addr = (
            self._exchange_address_from_token(token)
    ...
    def estimate_price_impact(
        ...
        if route == None:
            ...
            try:
                price_small_1 = self.get_raw_price(
                    route[0],
                    route[1],
                    fee=fee,
                )

                price_small_2 = self.get_raw_price(
                    route[1],
                    route[2],
                    fee=fee,
                )

                price_small = price_small_1*price_small_2
            except (ArithmeticError, BadFunctionCallOutput):
                # ArithmeticError is raised when `token_in` amount in the pool equals 0.
                # BadFunctionCallOutput is raised when the pool's contract for given `(token_in, token_out, fee)` hasn't been deployed
                return 1
